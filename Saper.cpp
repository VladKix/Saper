...
int field[100][100];
int field_p[100][100];
// -2 esli pole ne otkrito
// -1 esli stoit flag ili mina
// 0 - 8 esli kletka otkrita
...
void end_game(bool is_win = false) {
	...
	print_field();
	cout << "Vi " << (is_win ? "viigrali" : "proigrali") << "\nchtobi nachat novuy igru vvedite lubuy stroku:\n";
	string s;
	cin >> s;
}
...
int main()
{
	while (true) {
		...
		while (true) {
			...
			cout << "Enter comand:";
			/*
			\o x y najat na kletku
			\f x y postavit flag
			\n nachat zanovo
			\q zavershenie programmi
			*/
			string comand;
			cin >> comand;
			if (comand == "\\o") {
